    ...
    public static void processcode(string code)
    {
        switch (code)
        {
            ...
            case "$quest":
                ...
                HelperItems.save.AdvanceMainQuest(m.mainQuest[m.GetIndexForMainQuestByID(HelperItems.save.GetActiveMainQuestState().Quest.id)+1].id);
                break;
            case "$fixsave":
                //Plugin.fixerror = fixsave();
                break;
            ...
            case "$cheat.crystals":
                ...
                Plugin.ArchipelagoClient.SendMessage($"Player:{ServerData.SlotName} activated crystal dev cheat");
                HelperItems.save.AddItemToInventory(ItemManager.instance.GetItemAssetByName("Crystal_SpiritCrystal+2"), 10, true);
                break;
            case "$cheat.potion":
                ArchipelagoConsole.LogMessage("Adding 10 Greater Rejuvenation Potion to Inventory");
                Plugin.ArchipelagoClient.SendMessage($"Player:{ServerData.SlotName} activated potion dev cheat");
            ...
            case "$cheat.sex":
                ...
                Plugin.ArchipelagoClient.SendMessage($"Player:{ServerData.SlotName} activated affection dev cheat");
                HelperItems.save.AddItemToInventory(ItemManager.instance.GetItemAssetByName("Consumable_ChocolateCake"), 10, true);
                break;
            case "$cheat.warp":
                Plugin.ArchipelagoClient.SendMessage($"Player:{ServerData.SlotName} activated warp dev cheat");
            ...
            case "$cheat.1"://Ancient Temple Key
                Plugin.ArchipelagoClient.SendMessage($"Player:{ServerData.SlotName} activated keyitem dev cheat");
